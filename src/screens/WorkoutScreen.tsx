...
import { getTodayWorkouts, saveWorkout, deleteWorkout } from '../api/api';
...
export default function WorkoutScreen() {
  ...
  const selectPreset = (preset: (typeof PRESET_WORKOUTS)[0]) => {
    ...
    if (durationMin) {
      setKcalBurned(String(Math.round(preset.kcalPerMin * parseInt(durationMin, 10))));
    }
  };
  ...
  const onDurationChange = (val: string) => {
    ...
    if (preset && numeric) {
      setKcalBurned(String(Math.round(preset.kcalPerMin * parseInt(numeric, 10))));
    }
  };
      ...
      >
        ...
        {loading ? (
          ...
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptyText}>오늘 운동 기록이 없어요</Text>
          ...
          <View style={styles.modalCard}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
              {PRESET_WORKOUTS.map((p) => (
                <TouchableOpacity
                  key={p.name}
                  style={[styles.presetBtn, exerciseName === p.name && styles.presetBtnActive]}
            ...
            <View style={styles.inputRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <TextInput
                  ...
                  keyboardType="numeric"
                  value={kcalBurned}
                  onChangeText={(val) => setKcalBurned(val.replace(/[^0-9]/g, ''))}
                />
              </View>
            </View>
