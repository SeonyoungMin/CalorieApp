...
function MealRow({ item }: { item: MealCarryItem }) {
  ...
  const statusText = isOver
    ? `${Math.abs(remaining)}kcal 초과했어요 ⚠️`
    : remaining === 0
    ? '딱 맞게 드셨어요! 👏'
...
const styles = StyleSheet.create({
  ...
  settingsText: { fontSize: 12, color: COLORS.primary, fontWeight: '700' },
  mealRow: { marginBottom: 18 },
  mealHeader: {
    flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6,
  },
  mealEmoji: { fontSize: 18 },
  mealLabel: { fontSize: 14, fontWeight: '700', color: COLORS.text, flex: 1 },
  mealGoal: { fontSize: 13, fontWeight: '600', color: COLORS.text },
  gaugeBg: {
    height: 10, backgroundColor: '#E8EDF2', borderRadius: 5,
    overflow: 'hidden', marginBottom: 6,
  },
  gaugeFill: { height: '100%', borderRadius: 5 },
  statusText: { fontSize: 12, color: COLORS.success, fontWeight: '600', marginBottom: 4 },
  ...
  carryBadgeText: { fontSize: 11, color: '#1565C0', fontWeight: '600' },
  recoWrap: { marginTop: 2 },
  recoItem: { fontSize: 11, color: '#546E7A', lineHeight: 18 },
